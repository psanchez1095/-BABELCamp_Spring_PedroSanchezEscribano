...
public class FormacionServiceImp implements FormacionService {
	...
	@Override
	public List<Curso> cursoNoMatriculadoAlumno(String usuario) {
		List<Curso> list = daoCursos.findByAlumnoNoMatriculado(usuario);
		return list.size() > 0 ? list : null;
	}
	...
	@Override
	public List<Curso> cursosByRange(Date ini, Date end) {
		List<Curso> list = daoCursos.findByFechaInicioBetween(ini, end);
		return list.size() > 0 ? list : null;
	}
